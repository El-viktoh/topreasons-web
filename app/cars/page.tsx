...
export default function Cars() {
  ...
  const fetchCars = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("rentals").select("*").eq("type", "car");
    if (!error && data) {
      setRentals(data);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <section className="relative py-24 px-4 overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto text-center">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Find the perfect vehicle for your journey across Ghana.</p>
        </div>
      </section>
          ...
          {loading ? (
            ...
            <>
              <p className="text-muted-foreground mb-6 text-sm uppercase tracking-widest">
                {filteredRentals.length} vehicle{filteredRentals.length !== 1 ? "s" : ""} found
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-bold mb-2 uppercase tracking-wide">No Cars Found</h3>
              <p className="text-muted-foreground mb-6">
                {searchQuery ? "Try adjusting your search or filters" : "No cars are currently available. Check back soon!"}
              </p>
              {searchQuery && (
                <Button
                  variant="outline"
